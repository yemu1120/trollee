...
class DetailUI(Ui_mainWindow, QMainWindow, Write, Report, Template):
    ...
    def getFile(self):
        ...
        try:
            ...
            self.userVar = []
            self.userParams = []
            reportDate = time.strftime("%Y%m%d%H", time.localtime())
            self.example.clear()
    ...
    def openExcelReport(self):
        runTime = time.strftime("%Y%m%d%H", time.localtime())
        try:
            fname = self.fileName.text()
            if fname in ['请选择文件', '']:
                ex.console.clear()
    ...
    def openLog(self):
        try:
            ...
            if fname in ['请选择文件', '']:
                ...
                sss = 'r' + "'" + path + '/result/%s_info.log' % time.strftime('%Y-%m-%d') + "'"
                os.startfile(eval(sss))
        except Exception as e:
            print(e)
            ex.console.clear()
